  ...
  task :start do
    ...
    begin
      ...
      Dir.chdir(SOLR_PATH) do
        ...
        puts "#{SOLR_JVM_OPTIONS} -Dsolr.data.dir=#{SOLR_DATA_PATH} -Djetty.logs=#{SOLR_LOGS_PATH} -Djetty.host=127.0.0.1 -Djetty.port=#{SOLR_PORT} -jar start.jar"
        #pid = fork do
          #STDERR.close
        pid =   exec "java #{SOLR_JVM_OPTIONS} -Dsolr.data.dir=#{SOLR_DATA_PATH} -Djetty.logs=#{SOLR_LOGS_PATH} -Djetty.host=127.0.0.1 -Djetty.port=#{SOLR_PORT} -jar start.jar"
        #end
        sleep(5)
